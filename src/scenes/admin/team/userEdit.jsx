import { Alert, Box, Button, Checkbox, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, FormControlLabel, MenuItem, Select, TextField, Typography, useMediaQuery, useTheme } from '@mui/material';
import React, { useEffect, useState } from 'react'
import { tokens } from '../../../theme';
import { AdminPanelSettingsOutlined, LockOpenOutlined, SecurityOutlined } from '@mui/icons-material';
import { ChangeAccessLevel, UpdataUserInfo } from "../../../functions/admin/function";
...
export const UserEdit = (props) => {
  ...
  const [ShowPassword, setShowPassword] = useState(false);
    ...
    const handleFormSubmit = async (values) => {
      UpdataUserInfo(values, data.key);
      setOpen(false);
    };
  ...
  const checkoutSchema = yup.object().shape({
    ...
    desg: yup.string().required("Description is Required!"),
  });
  const initialValues = {
  ...
  return (
    <>
          ...
          <Formik
            ...
            {({
              ...
              <form onSubmit={handleSubmit}>
                <DialogContent>
                  ...
                  >
                    ...
                    <Select
                      ...
                      sx={{ gridColumn: "span 4" }}>
                        <MenuItem value={""} selected> Select Department </MenuItem>
                    ...
                    {/* <Select
                      ...
                      value={values.desg}
                      name="desg"
                      error={!!touched.desg && !!errors.desg}
                      helpertext={touched.desg && errors.desg}
                      sx={{ gridColumn: "span 4" }}>
                        <MenuItem value={""} selected> Select Desgination </MenuItem>
                ...
                <DialogActions>
                  <Button color='error' onClick={() => (setOpen(false))} sx={{marginRight: 'auto'}}>Cancel</Button>
                  <Button color={data.accountstatus === 1 ? 'success' : 'error'} onClick={() => ChangeAccessLevel(data.key, data.accountstatus, data.companyName, props.Alert)} >
                    <Typography color={colors.grey[100]} sx={{ ml: "5px" }}>
                      {data.accountstatus === 0 && "Disable"}
                      {data.accountstatus === 1 && 'Active'}
                      {data.accountstatus === 2 && 'Delete'}
                    </Typography>
                  </Button>
      ...
      {/* <Button
        ...
        variant='contained'
        onClick={() => ChangeAccessLevel(data.key, data.accountstatus, data.companyName, props.Alert)}
        color={
        ...
        borderRadius="4px"
      >
        {data.accountstatus === 0 && <AdminPanelSettingsOutlined />}
        {data.accountstatus === 1 && <SecurityOutlined />}
        {data.accountstatus === 2 && <LockOpenOutlined />}
        <Typography color={colors.grey[100]} sx={{ ml: "5px" }}>
          {data.accountstatus === 0 && "Active"}
          {data.accountstatus === 1 && 'Disable'}
          {data.accountstatus === 2 && 'Delete'}
        </Typography>
      </Button> */}
    </>
