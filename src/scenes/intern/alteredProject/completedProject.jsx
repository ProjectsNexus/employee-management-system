import { Box, Button, Card, CardActions, CardContent, CardHeader, LinearProgress, Divider, Link, Tooltip, Typography, useTheme } from "@mui/material";
  ...
  return (
    <Box m="20px">
      <Box display={"flex"} justifyContent={"space-between"}>
        ...
        <Typography variant="body1" display={"flex"} alignItems={"center"}> {ProjectData.filter(item => item.data.Status === 3).length } Projects in your history </Typography>
      </Box>
      {isLoad ? (
        ...
        <Box display="grid" gap="30px" gridTemplateColumns="repeat(1, minmax(0, 1fr))">
          {
            ProjectData.filter(item => item.data.Status === 3).length != 0 ? 
              (ProjectData.filter(item => item.data.Status === 3).map(item => (
                <Box key={item.key}>
                <Card fullWidth>
